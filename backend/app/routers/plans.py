...
async def change_plan(
    ...
    if new_plan.slug == 'free' and current_user.subscription_plan_id:
        result2 = await db.execute(
            select(SubscriptionPlan).filter(SubscriptionPlan.id == current_user.subscription_plan_id)
        )
        current_plan = result2.scalars().first()
        if current_plan and current_plan.price > 0:
            raise HTTPException(
                status_code=403,
                detail="Não é possível voltar para o plano Free"
            )
